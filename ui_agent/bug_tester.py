...
- For each issue in the list above, you MUST explicitly perform the test and decide if it fails or passes. 
- DO NOT finish the task until you have called `report_bug_result` for EVERY single issue in the checklist above.
- If you cannot find an element or a page mentioned in a test, report it as 'pass' but note in the detailed_report that the feature was not found.
- When you find a real UI/UX error (visual problem, broken link, missing feedback, usability blocker), IMMEDIATELY use the `report_bug_result` action with status='fail'.
- Your detailed_report for failed issues must be highly descriptive (3-5 sentences), explaining the exact flaw observed and specific guidance on how to fix it in code.

Once you have tested and reported ALL issues using `report_bug_result`, you must finish the task."""
